...
contract UniswapV2ArbHook is Ownable, ReentrancyGuard {
    ...
    function _executeArbitrageLogic(
        ...
        (address bestSecondaryDEX, uint256 bestOutput, uint256 profit) = _findBestArbitrageOpportunity(
            tokenOut, 
            tokenIn, 
            amountIn, 
            primaryDEX
    ...
    function _findBestArbitrageOpportunity(
        address tokenIn,
        address tokenOut,
        uint256 amountIn,
        address excludeDEX
    ) internal view returns (address bestDEX, uint256 bestOutput, uint256 profit) {
        bestOutput = 0;
        bestDEX = address(0);
        profit = 0;
